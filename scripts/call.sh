    ...
    "data": [
      [
        0,
        {
          ...
          "cypher_query": "LOAD CSV FROM '\''file://consumer_data.social_network.social_nodes.csv'\'' AS row MERGE (a:Actor {name: row[0], node_label: row[1]}) RETURN a.name, a.node_label"
        }
      ]
    ]
  }' \
